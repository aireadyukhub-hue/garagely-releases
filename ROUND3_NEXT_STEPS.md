# GarageDash — Round 3 updates: what I built + what you need to do
...
> Hosting note: GarageDash runs on **Cloudflare** now (Worker + Pages), not Netlify.
...
`GarageDash-Backend/migrations/0008_settings_presets_dashboardfix.sql` was run in the Supabase
...
```
cd GarageDash-Worker && wrangler deploy
```
...
```
cd GarageDash-Web && npm run build && wrangler pages deploy dist --project-name garagedash-app
```
URL: https://garagedash-app.pages.dev

### 4. Deploy the admin app
```
cd GarageDash-Admin && npm run build && wrangler pages deploy dist --project-name garagedash-admin
```
URL: https://garagedash-admin.pages.dev
