/**
 * GarageDash Backend — Cloudflare Worker
 * ------------------------------------
...
function generateLicenceKey(): string {
  ...
  return `GDSH-${seg()}-${seg()}-${seg()}`
}

// Accepts both new GDSH keys and legacy GRLY keys — old keys stay valid forever.
const KEY_RE = /^(?:GRLY|GDSH)-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}$/
...
async function createCheckout(request: Request, env: Env): Promise<Response> {
  ...
  const site = env.SITE_URL || 'https://garagedash.co.uk'
...
async function stripeWebhook(request: Request, env: Env): Promise<Response> {
  ...
  switch (stripeEvent.type) {
    case 'checkout.session.completed': {
      ...
      // A Checkout session can complete with the subscription still in its
      // trial period (no card charged yet). Only mark the licence 'active'
      // once Stripe says the subscription is actually active — otherwise
      // trial signups inflate MRR / "Active Subscriptions" on the admin
      // dashboard the moment someone starts a self-serve trial.
      const licenceStatus = sub.status === 'trialing' ? 'trial' : 'active'

      // If this email already has a licence (e.g. a trial tester converting from
      // the in-app prompt), UPGRADE that licence — keeps their existing
      ...
      if (prior.data?.id) {
        const { error } = await supabase.from('licences').update({
          status: licenceStatus, stripe_customer_id: customerId,
        ...
        console.log(`Licence updated to ${licenceStatus}: ${prior.data.key} for ${email}`)
        break
      }
      ...
      const { error } = await supabase.from('licences').insert({
        key, email, garage_name: garageName, status: licenceStatus,
...
async function sendLicenceEmail(env: Env, to: string, key: string, garageName: string): Promise<void> {
  ...
  const from = env.RESEND_FROM || 'GarageDash <info@getgaragely.com>'
  if (!apiKey || !to) {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;color:#111">
      <h2 style="color:#F4A523">Welcome to GarageDash 🔧</h2>
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from,
        to: [to],
        subject: 'Your GarageDash licence key — please save this email',
        html,
      }),
    })
...
async function sendResetEmail(env: Env, to: string, link: string): Promise<void> {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;color:#111">
      <h2 style="color:#F4A523">Reset your GarageDash password</h2>
      <p>We received a request to reset the password for your GarageDash account. Click below to choose a new one:</p>
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from,
        to: [to],
        subject: 'Reset your GarageDash password',
        html,
      }),
    })
...
const GARAGEDASH_GUIDE = `You are Timmy — a cheerful, slightly cheeky 10mm socket who is the in-app help
mascot for GarageDash (garage/workshop management software, desktop + web). GarageDash users are
...
YOUR JOB: answer questions about how to use GarageDash. Be brief and practical. Give step-by-step
...
- Help & Feedback: ask this assistant, or send Feedback/ideas or a Support request to the GarageDash team.
...
async function assistant(request: Request, env: Env): Promise<Response> {
  ...
  const messages = [
    { role: 'system', content: GARAGEDASH_GUIDE },
    ...history.map((h: any) => ({
...
function inviteEmailHtml(key: string, trialDays: number): string {
  const site = 'https://garagedash.co.uk'
  const app = 'https://app.garagedash.co.uk'
  ...
  <table role="presentation" align="center" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e6e8ec;">
    <tr><td style="background:#0f1117;padding:28px 24px;text-align:center;border-bottom:3px solid #F4A523;">
      <img src="${site}/garagedash-email-logo.png" alt="GarageDash" width="280" style="display:inline-block;width:280px;max-width:72%;height:auto;" />
    ...
    <tr><td style="padding:32px 32px 8px 32px;">
      <h1 style="margin:0 0 6px;font-size:22px;line-height:1.3;color:#16181d;">You're invited to try GarageDash free for ${trialDays} days</h1>
      <p style="margin:0 0 18px;font-size:15px;line-height:1.6;color:#5b6270;">No card needed. GarageDash is a <strong style="color:#16181d;">budget-friendly garage management system</strong> — bookings, job sheets, quotes, invoices, customers and vehicles, all in one place on desktop and in your browser.</p>
    ...
    <tr><td style="padding:16px 32px 26px;border-top:1px solid #eef0f3;">
      <p style="margin:0;font-size:12px;color:#9aa0ac;">GarageDash — budget-friendly garage management.</p>
    </td></tr>
  </table>
</div>`
}
...
async function sendInviteEmail(env: Env, to: string, key: string, trialDays: number): Promise<void> {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from,
      to: [to],
      subject: 'Your GarageDash free trial invite',
