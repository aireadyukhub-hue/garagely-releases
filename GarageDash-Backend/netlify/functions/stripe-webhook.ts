...
async function sendLicenceEmail(to: string, key: string, garageName: string): Promise<void> {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;color:#111">
      ...
      <ol>
        <li>Download GarageDash from <a href="https://garagely.netlify.app">garagely.netlify.app</a> (or use it in your browser at <a href="https://garagely-app.netlify.app">garagely-app.netlify.app</a>).</li>
  ...
  try {
    const res = await fetch('https://api.sendgrid.com/v3/mail/send', {
      ...
      body: JSON.stringify({
        ...
        from: { email: from, name: 'GarageDash' },
...
export const handler: Handler = async (event) => {
  ...
  switch (stripeEvent.type) {
    ...
    case 'checkout.session.completed': {
      ...
      // trial signups inflate MRR / "Active Subscriptions" on the dashboard.
