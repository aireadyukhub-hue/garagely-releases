...
const SITE_URL = 'https://garagedash.co.uk'
const WEB_URL = 'https://app.garagedash.co.uk'
...
You're invited to try GarageDash free for ${r.trialDays} days — no card needed.

GarageDash is a budget-friendly garage management system: bookings, job sheets, quotes, invoices, customers and vehicles, on desktop and in your browser.
          ...
          {result ? (
            <div className="py-2">
              ...
              <div className="flex gap-2">
                ...
                <a
                  href={`mailto:${encodeURIComponent(result.email)}?subject=${encodeURIComponent('Your GarageDash trial invite')}&body=${encodeURIComponent(buildInvite(result))}`}
                  className="flex-1 bg-[#2A2D35] hover:bg-[#353841] text-white rounded-xl px-4 py-2.5 text-sm flex items-center justify-center gap-2"
                >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : `Invite to try GarageDash for ${trialDays} days`}
              </button>
            </form>
          )}
        </div>
